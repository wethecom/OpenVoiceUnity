...
namespace OpenVoiceSharp
{
    public sealed class VoiceChatInterface : IDisposable
    {
        ...
        public bool IsSpeaking(byte[] pcmData)
        {
            ...
            return VoiceActivityDetector.HasSpeech(pcmData);
        }
        ...
        public void Dispose()
        {
            ...
            if (OpusEncoder is IDisposable opusEncoderDisposable)
                opusEncoderDisposable.Dispose();
            if (OpusDecoder is IDisposable opusDecoderDisposable)
                opusDecoderDisposable.Dispose();
            if (Denoiser is IDisposable denoiserDisposable)
                denoiserDisposable.Dispose();
            if (VoiceActivityDetector is IDisposable vadDisposable)
                vadDisposable.Dispose();
            IsDisposed = true;
        }
