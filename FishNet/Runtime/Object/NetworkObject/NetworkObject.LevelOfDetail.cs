...
namespace FishNet.Object
{
    public partial class NetworkObject : MonoBehaviour
    {
        ...
        {
            //PROSTART
            // TODO be sure to check for NetworkTransforms once they are supported.
            bool canUseLevelOfDetail = _useLevelOfDetail && _enablePrediction && ObserverManager.UseLevelOfDetail && (!_useRootLevelOfDetail || !IsNested);
            bool canEnable = canUseLevelOfDetail && Observers.Count != 0;

            if (canEnable == ServerIsLevelOfDetailInitialized)
                return;

            if (canEnable)
            {
                ObserverManager.AddLevelOfDetailNetworkObject(this);
                ObserverLevelOfDetailDivisors = CollectionCaches<NetworkConnection, uint>.RetrieveDictionary();
                
                //Add all current observers at LOD 0.
                foreach (NetworkConnection conn in Observers)
                    ObserverLevelOfDetailDivisors[conn] = DEFAULT_LEVEL_OF_DETAIL_INDEX;
            }
            else
            {
                ObserverManager.RemoveLevelOfDetailNetworkObject(this);
                CollectionCaches<NetworkConnection, uint>.StoreAndDefault(ref ObserverLevelOfDetailDivisors);
            }

            ServerIsLevelOfDetailInitialized = canEnable;
            //PROEND
        }
        ...
        {
            //PROSTART
            ObserverLevelOfDetailDivisors?.Clear();
            //PROEND
        }
        ...
        {
            //PROSTART
            if (ObserverLevelOfDetailDivisors != null)
                ObserverLevelOfDetailDivisors[connection] = DEFAULT_LEVEL_OF_DETAIL_INDEX;
            //PROEND
        }
        ...
        {
            //PROSTART
            ObserverLevelOfDetailDivisors?.Remove(connection);
            //PROEND
        }
    }
}