...
namespace FishNet.Managing.Observing
{
    ...
    public sealed partial class ObserverManager : MonoBehaviour
    {
        ...
        //PROSTART
        /// <summary>
        /// Smallest divisor to use with level of detail.
        /// </summary>
        internal uint SmallestLevelOfDetailTickDivisor { get; private set; }
        /// <summary>
        ...
        /// <summary>
        /// Largest divisor to use with level of detail.
        /// </summary>
        private uint _largestLevelOfDetailTickDivisor;
        //PROEND
        ...
        {
            //PROSTART
            //Already initialized -- return current value.
            if (_levelOfDetailInitialized)
                return _useLevelOfDetail;

            if (!_useLevelOfDetail)
                return false;

            /* If Inserted prediction order disallow LOD.
             * The default order is Appended. */
            if (!_networkManager.PredictionManager.IsAppendedStateOrder)
            {
                _networkManager.LogError($"Level of detail can only be used with prediction appended order. If you are not using prediction you may safely change to this setting on this value on the PredictionManager.");
                return false;
            }

            //Limit the number of values for the users sake to cover them from poor performance.
            const int maximumDistances = 20;
            if (_levelOfDetailDistances == null || _levelOfDetailDistances.Count < 1 || _levelOfDetailDistances.Count > maximumDistances)
            {
                _networkManager.LogError($"Level of detail distances cannot be null and must have between 1 and {maximumDistances} values.");
                return false;
            }

            _maximumLevelOfDetailInterval = Mathf.Clamp(_maximumLevelOfDetailInterval, MINIMUM_LEVEL_OF_DETAIL_SEND_INTERVAL, MAXIMUM_LEVEL_OF_DETAIL_SEND_INTERVAL);
            SetLevelOfDetailRecalculationDuration(_levelOfDetailUpdateDuration);

            int entryCount = _levelOfDetailDistances.Count;

            _levelOfDetailTickDivisors.Clear();

            foreach (float lodDistance in _levelOfDetailDistances)
                _levelOfDetailDistancesSquared.Add(lodDistance * lodDistance);

            _levelOfDetailDistancesSquared.Sort();

            /* Increase the entry count and skip over the first
             * entry. This will still add 3 level of details, while
             * the first level of detail (no lod applied) will be
             * added manually as 1 tick.
             *
             * With 3 entries the exponentialTimes will be:
             *      0.222
             *      0.889
             *      2.000
             * */
            ...
            //1 tick -- no LOD.
            _levelOfDetailTickDivisors.Add(NetworkObject.DEFAULT_LEVEL_OF_DETAIL_INDEX);

            TimeManager timeManager = _networkManager.TimeManager;

            //Difference between original tick value and the value which was prime.
            uint primeDifference = 0;

            for (int i = 1; i < entryCount; i++)
            {
                //Index 0 
                // Normalized index (0 to 1)
                double t = (double)i / (entryCount - 1);

                // Apply power curve: t^exponent * maximumDelay
                const double exponent = 2f;
                double exponentialTime = Math.Pow(t, exponent) * _maximumLevelOfDetailInterval;

                /* The difference from the previous iteration is
                 * added on to maintain the minimum delay between ticks. */
                uint ticks = timeManager.TimeToTicks(exponentialTime, TickRounding.RoundUp) + primeDifference;
                //Get the first prime of ticks.
                uint primeTick = PrimeNumberFinder.GetNextPrime(ticks);

                primeDifference = primeTick - ticks;

                _levelOfDetailTickDivisors.Add(primeTick);
            }

            //Index 1 is used because 0 is '1', for no LOD.
            SmallestLevelOfDetailTickDivisor = _levelOfDetailTickDivisors[1];
            _largestLevelOfDetailTickDivisor = _levelOfDetailTickDivisors[^1];
            
            _levelOfDetailInitialized = true;
        ...
        //PROSTART
        /// <summary>
        /// Gets the tick to use for level of detail calculations using the current localtick.
        /// </summary>
        internal uint GetLevelOfDetailTick()
        {
            /* LocalTick is set by the TimeManager once
             * per tick to prevent excessive lookups during
             * LOD calculations. */
            uint result = LocalTick % _largestLevelOfDetailTickDivisor;
            return result == 0 ? _largestLevelOfDetailTickDivisor : result;
        }

        
        /// <summary>
        /// Gets the tick to use for level of detail calculations using a provided tick.
        /// </summary>
        internal uint GetLevelOfDetailTick(uint tick)
        {
            uint result = tick % _largestLevelOfDetailTickDivisor;
            return result == 0 ? _largestLevelOfDetailTickDivisor : result;
        }
        ...
        /// </summary>
        private byte GetLevelOfDetailIndexUnsafe(float sqrDistance)
        {
        ...
        /// </summary>
        private void UpdateLevelOfDetails()
        {
            if (!_useLevelOfDetail || _networkManager == null || !_networkManager.IsServerStarted)
                return;
        ...
        private bool TryGetLevelOfDetailIndex(Vector3 position, NetworkConnection connectionToCompare, out byte levelOfDetailIndex)
        {
            ...
            {
                float sqrDistance = (position - connectionObjects.transform.position).sqrMagnitude;
        ...
        /// </summary>
        internal uint GetLocalLevelOfDetailDivisorTickUnsafe(Vector3 position)
        {
